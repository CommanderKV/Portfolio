...
def home():
    with logfire.span("GET /"):
        logfire.info("Rendering home page")
...
def projects():
    with logfire.span("GET /projects"):
        logfire.info("Rendering projects page")
        return render_template(
            "projects.html", 
            projects=REPOS,
            account=GITHUB_ACCOUNT,
...
def contact():
    with logfire.span("GET /contact"):
        logfire.info("Rendering contact page")
        return render_template(
            "contact.html", 
            contact=CONTACT_INFO
        )
...
def skills():
    with logfire.span("GET /skills"):
        logfire.info("Rendering skills page")
        return render_template(
            "skills.html",
            skills=getSkills(),
            contact=CONTACT_INFO
...
def apiContact():
    with logfire.span("POST /api/contact"):
        # Get data from the request
        data: dict = request.json
        errors = {}
        logfire.info("Received contact form data", data=data)
        
        # Validating name data
        if not data.get("name"):
            errors["name"] = "Name is required"
            logfire.info("Name is required")
        ...
        if not data.get("email"):
            ...
            logfire.info("Email is required")
        ...
        if not data.get("message"):
            ...
            logfire.info("Message is required")
